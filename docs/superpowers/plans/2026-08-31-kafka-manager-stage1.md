...
Консьюмер-группы и лаг, управление топиками, просмотр сообщений — этапы 2, 3 и 4,
каждый со своей спекой.

## Отклонения от плана при выполнении

Зафиксировано по факту, чтобы документ не расходился с кодом:

1. **CSS в шаблоне, не в style.css.** В проекте стили страниц живут inline
   в `<style>` самого шаблона (как `grants.html`), кнопки — бутстраповские `.btn`.

2. **Пункт меню требует `<nav>`.** Правила висят на `.gp-sb nav a`, а блок
   Data Flow был обычным `div` с заглушками.

3. **Отдельная вкладка «Подключения».** План откладывал форму до этапа 3 —
   без неё вкладкой нельзя пользоваться. Сделана сразу: `/kafka/connections`,
   `templates/kafka_connections.html`, `static/js/kafka_connections.js`,
   в меню — раздел Kafka с пунктами «Подключения» и «Обзор кластера».

4. **kafka-python 3.x отличается от 2.x.** Поймано на живом кластере:
   * ожиданием брокеров управляет `bootstrap_timeout_ms`, а не
     `request_timeout_ms` (иначе вкладка висит 30 с);
   * `describe_topics()` и `list_topics()` шлют MetadataRequest с `topics=None`,
     что часть брокеров отвергает («All topics must not be None») — имена
     топиков берём у консьюмера и передаём явно;
   * `describe_topics()` возвращает весь ответ метаданных, а не список;
   * поля переименованы: `topic` → `name`, `partition` → `partition_index`,
     `leader` → `leader_id`, `replicas` → `replica_nodes`, `isr` → `isr_nodes`,
     `node_id` → `broker_id`. `build_overview` читает оба набора.

   Урок: фикстуры для чужого API надо сверять со схемами библиотеки,
   а не сочинять — иначе тест подтверждает собственную выдумку.
